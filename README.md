...
**问题**：CentOS 7 系统版本较低，**不支持 Node.js 18+**

宝塔面板会提示：`操作系统版本过低，该Node版本不兼容此操作系统`

**解决方案（三选一）**：

**方案一：降级前端构建工具（推荐）⭐**
- 修改 `client/package.json`，降级 Vite 和相关依赖
- 使用 Node.js 16.x 构建
- 详见下方"CentOS 7 兼容性配置"

**方案二：升级操作系统**
- 升级到 CentOS 8 / Rocky Linux 8 / AlmaLinux 8
- 或使用 Ubuntu 20.04+

**方案三：使用 Docker 部署**
- 在 Docker 容器中使用 Node.js 18+
- 不受宿主机系统限制
...
如果必须在 CentOS 7 上部署，需要降级前端依赖：

#### 1. 修改 `client/package.json`
...
后端项目可以使用 Node.js 16.x 运行，没有问题。

---

### 推荐系统版本

| 操作系统 | 版本 | Node.js 18+ 支持 |
|---------|------|-----------------|
| CentOS | 7.x | ❌ 不支持 |
| CentOS Stream | 8+ | ✅ 支持 |
| Rocky Linux | 8+ | ✅ 支持 |
